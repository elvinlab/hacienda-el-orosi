...
router.get('/historial-cuotas/:id', [md_auth.authenticated, getFeesByLend]);
...
router.delete(
  '/eliminar-prestamo/',
  [check('lendId', 'falta el identificador del prestamo').not().isEmpty(), validate_fields],
  md_auth.authenticated,
  deleteLend
);

router.get('/prestamos/:status', md_auth.authenticated, getLendsByStatus);
router.get('/colaborador/prestamos/:id', md_auth.authenticated, getLendsByCollaborator);
