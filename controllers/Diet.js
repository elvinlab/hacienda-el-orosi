...
const addAliment = async (req, res = response) => {
  if (req.user.role === 'Dueño' || req.user.role === 'Encargado del ganado') {
    ...
    const { product_code, quantity_supplied } = req.body;

    try {
      const findProductByCode = await Product.findOne({
        active_num: product_code
      });
      ...
      aliment.product = findProductByCode._id;
...
const updateDiet = async (req, res = response) => {
  if (req.user.role === 'Dueño' || req.user.role === 'Encargado del ganado') {
    ...
    await Diet.findByIdAndUpdate({ _id: dietId }, { diet_name, description, animal }, (err) => {
      if (err) {
        res.status(400).json({
          status: false,
          msg: 'Por favor contacté con un ING en Sistemas para más información.'
...
const updateAliment = async (req, res = response) => {
  if (req.user.role === 'Dueño' || req.user.role === 'Encargado del ganado') {
    ...
    await Aliment.findByIdAndUpdate({ _id: alimentId }, { quantity_supplied }, (err, aliment) => {
      if (err || !aliment) {
        return res.status(400).json({
          status: false,
          msg: 'Por favor contacté con un ING en Sistemas para más información.'
